...
개발 결과 보고서와 `skill-qa.md`를 Read한 후, 4단계 QA 실행:

### Tier 0: 자동화 테스트 (Automated — 최우선)

Bash로 자동화 테스트 실행 (빌드 검증 포함):

```bash
cd extension && npm run typecheck && npm run build && npm test 2>&1
```

**Tier 0 결과 처리:**

- **ALL PASS** → "✅ Tier 0: N/N 통과" 기록, Tier 1로 계속
- **FAIL 발생** → 실패한 테스트를 **자동으로 CRITICAL 버그로 등록**:
  - Bug ID: `T0-{N}` (T0-1, T0-2, ...)
  - 심각도: **CRITICAL** (자동 테스트 실패 = 배포 불가)
  - 설명: 실패한 테스트 이름 + 에러 메시지
  - 재현 경로: `npm test` 실행
  - 영향 범위: 해당 테스트가 검증하는 컴포넌트/서비스
- **빌드 실패** (`typecheck` 또는 `build`) → **CRITICAL** 버그로 등록, Tier 1~3 스킵

> ⚠️ Tier 0 FAIL 시에도 Tier 1~3은 계속 실행. 자동 테스트가 잡지 못하는 수동 QA 항목이 있으므로.
...
- **PASS**: CRITICAL 0건 (Tier 0 포함), MAJOR 0건 → Gate 2 자동 통과, Gate 3으로 직행
- **CONDITIONAL_PASS**: CRITICAL 0건, MAJOR N건 → 수정 후 재QA
- **FAIL**: CRITICAL N건 (Tier 0 테스트 실패 포함) → 즉시 수정 필수
```
...
### 자동 테스트 재검증 (T0 버그가 있었던 경우)

T0-{N} 버그(자동 테스트 실패)가 수정 대상에 포함되어 있다면, 수정 후 반드시 재검증:

```bash
cd extension && npm test 2>&1
```

- **ALL PASS** → T0 버그 상태를 "✅ 수정 완료"로 업데이트
- **여전히 FAIL** → 실패 테스트 정보를 에이전트에게 전달하여 **재수정 시도** (최대 2회)
- 2회 재수정 후에도 실패 → "⚠ 미수정"으로 기록, 사장에게 Gate 3에서 판단 위임
